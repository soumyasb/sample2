...
class AddCase extends Component {
  ...
  static getDerivedStateFromProps(props, prevState) {
    const createCaseObj = props.cases.createCaseObj;

    if (createCaseObj && createCaseObj !== prevState.createCaseObj) {
      if (createCaseObj.CaseNumber) {
        props.history.push(`/caseDetails/CaseNumber/${createCaseObj.CaseNumber}`);

        return null;
      }

      props.getCustomerD26Info(props.cases.createCaseObj.DLNumber);

      return {
        createCaseObj: createCaseObj,
        caseReason: createCaseObj.CD_RSN,
        caseReferral: createCaseObj.CD_REFR_SRCE_TYP,
        caseCertification: createCaseObj.CD_ENDR,
        hearingTypes: props.cases.hearingTypesList,
        caseReasons: props.cases.caseReasonsList,
        caseReferrals: props.cases.caseReferralsList,
        caseCertifications: props.cases.caseCertificationsList,
        value: createCaseObj.FRCaseNumber,
        phoneNumber: createCaseObj.PhoneNumber
      };
    }

    if (props.cases.customerD26Info !== prevState.customerD26Info) {
      return { 
        customerD26Info: props.cases.customerD26Info 
      };
    }
  ...
  handleChange(e, listType) {
    let obj = null;
    const createCaseObj = this.state.createCaseObj;

    switch (listType) {
      case 'HRNG_TYP':
        createCaseObj.HRNG_TYP = e;
        break;
      case 'CASE_RSN':
        createCaseObj.CASE_RSN = e;
        break;
      case 'CASE_RFL':
        createCaseObj.CASE_RFL = e;
        break;
      case 'CASE_CERT':
        createCaseObj.CASE_CERT = e;
        break;
      default:
        break;
    }
